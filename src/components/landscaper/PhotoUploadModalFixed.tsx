...
function PhotoUploadInner({ 
  ...
  const handleUpload = async () => {
    ...
    try {
      if (!isOnline) {
        // Store offline
        await offlinePhotoStorage.storePhoto(jobId, 'before', beforeFile, {
          gps: await getGPSLocation(),
          deviceInfo: getDeviceInfo()
        })
        await offlinePhotoStorage.storePhoto(jobId, 'after', afterFile, {
          gps: await getGPSLocation(),
          deviceInfo: getDeviceInfo()
        })
        
        showToast('Photos saved offline. Will sync when online.', 'success')
        onClose()
        onSuccess()
        setBeforeFile(null)
        setAfterFile(null)
        ...
        <DialogContent className="bg-black/95 backdrop-blur border border-green-500/30 text-white max-w-md shadow-2xl shadow-green-500/20">
          <DialogHeader>
            <DialogTitle className="text-green-400 flex items-center gap-2 text-xl">
              ...
              Upload Job Photos
              {!isOnline && <WifiOff className="w-5 h-5 text-amber-400" />}
            </DialogTitle>
          </DialogHeader>
          
          {!isOnline && (
            <div className="bg-amber-900/20 border border-amber-500/30 rounded p-3 text-amber-300 text-sm">
              You're offline. Photos will be saved locally and synced when connection is restored.
            </div>
          )}
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {loading ? 'Processing...' : isOnline ? 'Upload & Complete Job' : 'Save Offline'}
              </Button>
            </div>
          </div>
        </DialogContent>
