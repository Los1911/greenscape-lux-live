...
export function JobMessageButton({ 
  ...
  // Mark messages as read when opening messaging
  ...
  return (
    <>
      <Button
        ...
        className={`flex items-center gap-2 border-emerald-500/30 text-emerald-200 hover:bg-emerald-900/30 relative ${className}`}
      >
        ...
        {isActive ? (clientName ? `Message ${clientName}` : 'Message Client') : 'View Messages'}
        {!isActive && <Lock className="w-3 h-3 ml-1 opacity-50" />}
      </Button>
...
export default JobMessageButton;
