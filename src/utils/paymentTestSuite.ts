...
export class PaymentTestSuite {
  ...
  private async testStripeConnectAccount() {
    ...
    try {
      // Use Supabase functions.invoke pattern instead of /api/ route
      // This calls the stripe-connect-onboarding edge function
      const { createClient } = await import('@supabase/supabase-js');
      const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
      const supabaseKey = import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY;
      
      if (!supabaseUrl || !supabaseKey) {
        this.addResult(test, false, 'Missing Supabase configuration');
        return;
      }
      ...
      const { data, error } = await supabase.functions.invoke('stripe-connect-onboarding', {
        body: {
          ...
          userId: 'test-landscaper-123'
        }
      });

      if (error) {
        this.addResult(test, false, `Stripe Connect account creation failed: ${error.message}`);
        return;
      }
      ...
      if (data?.accountId && data?.onboardingUrl) {
        ...
        this.addResult(test, false, `Stripe Connect error: ${data.error}`);
      } else {
